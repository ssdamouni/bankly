...
describe("GET /users", function() {
  ...
  test("BUG #2 should only basic information of all users", async function() {
    const response = await request(app)
      .get("/users")
      .send({ _token: tokens.u1 });
    expect(response.body).toEqual({
...
describe("PATCH /users/[username]", function() {
  ...
  test(" BUG #5: should patch data if it is their own information", async function() {
    const response = await request(app)
      .patch("/users/u1")
      .send({ _token: tokens.u1, first_name: "new-fn1" }); // u3 is the user
    expect(response.statusCode).toBe(200);
    expect(response.body.user).toEqual({
      username: "u1",
      first_name: "new-fn1",
...
describe("DELETE /users/[username]", function() {
  ...
  test("should return 404 if user can't be found", async function() {
    const response = await request(app)
      .delete("/users/fake")
