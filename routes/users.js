...
//BUG 5: Removed requireAdmin from this routes middleware
router.patch('/:username', authUser, requireLogin, async function(
  ...
  try {
    //BUG 7: added validation of the incoming data
    const validator = jsonschema.validate(req.body, userUpdateSchema);
    if (!validator.valid) {
      throw new ExpressError('information on invalid field', 401);
    }
...
router.delete('/:username', authUser, requireAdmin, async function(
  ...
  try {
    //BUG #6: Fixed by add the await keyword in front of User
